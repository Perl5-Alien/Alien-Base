...
L<Alien::Base> is the subclass of C<Alien::MyLibrary>. In this context, L<Alien::Base> has two distinct uses. First it is used by C<Alien::MyLibrary> to provide the build information/flags for building C<Some::Module::MyLibrary>. Secondly it is use (again through C<Alien::MyLibrary>) to allow run-time access to F<libmylibrary.so> to C<Some::Module::MyLibrary>.
...
C<Alien::MyLibrary> must also be called by C<Some::Library::MyLibrary> before C<DynaLoader::bootstrap> or C<XSLoader::load>. This does the run-time provisioning so that when the XS file is loaded, it can find L<libmylibrary.so>. It does this by appending to C<$ENV{LD_RUN_PATH}> or your system's equvialent environment variable (PLEASE file a bug if this should fail on your system).
...
The C<use> directive is recommended, however if you must use C<require> then be sure to call the C<import> method too.
