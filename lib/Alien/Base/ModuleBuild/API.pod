...
This key should be specified when the filename of the tarball is known, in which case such a file is downloaded from the given C<host> and C<location>. When the filename contains a version number, it may be useful to also specify a C<pattern> with capture group, in order to allow the extraction of the version number from the filename.
