...
example, your C<Build.PL> might look like this:
 ...
 # --binary is required to get the same results on Windows
 # where line endings are different.  Not all versions of
 # patch support this option so it should only be used where
 # necessary.
 my $patch = $^O eq 'MSWin32' ? 'patch --binary' : 'path';
